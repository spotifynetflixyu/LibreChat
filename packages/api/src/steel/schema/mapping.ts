...
export const steelSourceSchemaMappings: readonly SteelSourceSchemaMappingEntry[] = [
  ...
  },
  {
    ...
    sourceLabels: ['來源區塊', 'cutting_category'],
    canonicalKey: 'cutting_category',
    target: 'steel.cutting_prices.cutting_category',
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'],
    valueType: 'string',
  },
  {
    sourceFile: '切工價錢-raw.xlsm',
    sourceLabels: ['品項/尺寸'],
    canonicalKey: 'spec_text',
    target: 'steel.cutting_prices.spec_text',
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'],
    valueType: 'string',
  },
  {
  ...
  },
  {
    sourceFile: '切工價錢-raw.xlsm',
    sourceLabels: ['tier B'],
    canonicalKey: 'unit_price_b',
    target: 'steel.cutting_prices.unit_price_b',
    allowedFor: ['admin_import', 'price_lookup'],
    valueType: 'money',
  },
  {
    ...
    sourceLabels: ['備註'],
    canonicalKey: 'notes',
    target: 'steel.cutting_prices.notes',
    allowedFor: ['admin_import', 'price_lookup'],
    valueType: 'string',
  },
  {
    sourceFile: '切工價錢-v4.4-normalized.xlsx',
    sourceLabels: ['normalized_spec_text'],
    canonicalKey: 'normalized_spec_text',
    target: 'steel.cutting_prices.normalized_spec_text',
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'],
    valueType: 'string',
  },
  ...[
    ['thickness_axis', 'cutting_thickness_axis'],
    ['thickness_mm_values', 'cutting_thickness_mm_values'],
    ['thickness_mm_min', 'cutting_thickness_mm_min'],
    ['thickness_mm_max', 'cutting_thickness_mm_max'],
  ].map(([sourceLabel, canonicalKey]) => ({
    sourceFile: '切工價錢-v4.4-normalized.xlsx',
    sourceLabels: [sourceLabel],
    canonicalKey,
    target: `steel.cutting_prices.${sourceLabel}`,
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'] as const,
    valueType: sourceLabel === 'thickness_axis' ? ('enum' as const) : ('number' as const),
  })),
  {
    sourceFile: '切工價錢-v4.4-normalized.xlsx',
    sourceLabels: ['spec_selector_json'],
    canonicalKey: 'cutting_spec_selector',
    target: 'steel.cutting_prices.spec_selector',
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'],
    valueType: 'string',
    note: 'Versioned JSON selector for structured price-only cutting-price matching; no supplement/manual-review rows are retained.',
  },
  {
    sourceFile: '切工價錢-v4.4-normalized.xlsx',
    sourceLabels: ['cut_type'],
    canonicalKey: 'cut_type',
    target: 'steel.cutting_prices.cut_type',
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'],
    valueType: 'string',
  ...
  ...priceTierCodes.map((tier) => ({
    ...
    target: `steel.cutting_prices.unit_price_${tier.toLowerCase()}`,
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'] as const,
    valueType: 'money' as const,
    customerTierCode: tier,
  })),
  {
    sourceFile: '切工價錢-v4.4-normalized.xlsx',
    sourceLabels: ['calculation_rule'],
    canonicalKey: 'cutting_calculation_rule',
    target: 'steel.cutting_prices.calculation_rule',
    allowedFor: ['admin_import', 'calculator_context', 'price_lookup'],
    valueType: 'string',
  },
  {
