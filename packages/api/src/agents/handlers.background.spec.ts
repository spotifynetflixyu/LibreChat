...
describe('createToolExecuteHandler — background tool calls', () => {
  it('forwards host custom events from a background delegate OCR call', async () => {
    const events: Array<{ name: string; payload: unknown }> = [];
    const delegateTool = {
      name: 'delegate_ocr',
      description: 'delegate OCR',
      schema: z.object({ fileKeys: z.array(z.string()) }),
      invoke: async (_input: unknown, config: Record<string, unknown>) => {
        const configurable = config.configurable as {
          hostCustomEventDispatcher?: (name: string, payload: unknown) => Promise<void>;
        };
        await configurable.hostCustomEventDispatcher?.('delegate_ocr_stream', { delta: 'page 1' });
        return { content: 'OCR complete' };
      },
    } as unknown as StructuredToolInterface;
    const handler = createToolExecuteHandler({
      loadTools: async () => ({
        loadedTools: [delegateTool],
    ...
    const graph = {
      handlerRegistry: {
        getHandler: () => ({
          handle: async (name: string, payload: unknown) => events.push({ name, payload }),
        }),
      },
    };
        ...
        toolCalls: [
          {
            id: 'call_bg_delegate',
            name: 'delegate_ocr',
            args: { fileKeys: ['file:drawing-1'], run_in_background: true },
          },
        ],
        agentId: 'agent_delegate',
        configurable: {
          ...buildConfig(['delegate_ocr']),
          delegateOcrStreaming: true,
        },
        metadata: { thread_id: 'exec_convo_delegate' },
        resolve: jest.fn(),
        reject: jest.fn(),
      } as unknown as Parameters<typeof handler.handle>[1],
      undefined,
    ...
    expect(events).toEqual([{ name: 'delegate_ocr_stream', payload: { delta: 'page 1' } }]);
  });
