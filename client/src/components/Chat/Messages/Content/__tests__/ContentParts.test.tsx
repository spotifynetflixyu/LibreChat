  ...
  return {
    MessageContext: {
      Provider: ({
        ...
        children: React.ReactElement<{ idx?: number; markdownTableBaseIndex?: number }>;
        value: { partIndex: number; markdownTableBaseIndex: number };
      }) =>
        react.cloneElement(children, {
          idx: value.partIndex,
          markdownTableBaseIndex: value.markdownTableBaseIndex,
        }),
    },
    SearchContext: {
...
jest.mock('../Part', () => ({
  ...
  default: ({
    ...
    <div
      ...
      data-index={idx}
      data-table-base-index={markdownTableBaseIndex}
    />
  ),
}));
...
describe('ContentParts — activity phase state', () => {
  ...
  it('keeps Markdown table indices message-wide across activity phase slices', () => {
    ...
    const phase = {
      type: ContentTypes.ACTIVITY_LABEL,
      [ContentTypes.ACTIVITY_LABEL]: 'Checked the first table',
      activity_label_type: 'phase',
      activity_start_index: 0,
      activity_count: 1,
      pending: false,
    ...
    render(<ContentParts {...baseProps} content={[table('one'), phase, table('two')]} />);

    const tables = screen.getAllByTestId(`real-part-${ContentTypes.TEXT}`);
    expect(tables[0]).toHaveAttribute('data-table-base-index', '0');
    expect(tables[1]).toHaveAttribute('data-table-base-index', '1');
  });
