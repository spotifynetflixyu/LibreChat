...
const countWithin = (
  node: MdastNode,
  counts: { code: number; artifact: number; table: number; mermaid: number },
): void => {
...
export function splitMarkdownIntoBlocks(content: string): MarkdownBlock[] {
  ...
  if (children.length === 0) {
    return [{ raw: content, codeBlockCount: 0, artifactCount: 0, tableCount: 0, mermaidCount: 0 }];
  }
  ...
  for (const node of children) {
    ...
    const counts = { code: 0, artifact: 0, table: 0, mermaid: 0 };
