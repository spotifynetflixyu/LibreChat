...
import type { ExtendedFile, FileSetter, MarkdownTableComment } from '~/common';
import {
...
export default function useSteering({
  ...
  const submitSteer = useCallback(
    (text: string, steerFiles?: TMessage['files'], context?: QueuedMessageContext): boolean => {
      ...
      steerMessage(
        { conversationId, text: preparedText, ...(files && { files }) },
        {
          onSuccess: (response) => {
          ...
          onError: (error) => {
            ...
            if (code === 'NO_ACTIVE_RUN') {
              ...
              if (
                ...
                sendNow(preparedText, files ?? [], context == null ? undefined : carried) === false
              ) {
                enqueue(preparedText, { files, ...carried });
              }
              return;
            }
            if (
              ...
              if (!isSubmittingRef.current) {
                if (
                  sendNow(preparedText, files ?? [], context == null ? undefined : carried) ===
                  false
                ) {
                  enqueue(preparedText, { files, ...carried });
                }
