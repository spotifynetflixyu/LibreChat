...
describe('useQueueDrain', () => {
  ...
  it('passes a queued message`s attachments through as overrideFiles', async () => {
    ...
    expect(ask).toHaveBeenCalledWith(
      { text: 'with media' },
      { ...emptyOverrides, overrideFiles: files },
    );
  });
  ...
  it('passes the queued Markdown comments explicitly instead of draining newer composer comments', async () => {
    const markdownTableComments = [
      {
        id: 'message-1:0:1:2',
        conversationId: CONVO_ID,
        messageId: 'message-1',
    ...
    const { ask, setters } = setup(({ set }) => {
      set(store.queuedMessagesByConvoId(CONVO_ID), [
        { ...queuedMessage('q1', 'with table comment'), markdownTableComments },
      ]);
    });
    ...
    await waitFor(() => expect(ask).toHaveBeenCalledTimes(1));
    expect(ask).toHaveBeenCalledWith(
      { text: 'with table comment' },
      { ...emptyOverrides, overrideMarkdownTableComments: markdownTableComments },
    );
  });
