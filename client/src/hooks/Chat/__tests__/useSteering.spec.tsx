...
const markdownTableComment: MarkdownTableComment = {
  id: 'message-1:0:1:2',
  conversationId: CONVO_ID,
  messageId: 'message-1',
  messageTimestampLabel: '10:00',
  markdownIndex: 0,
  markdownLabel: 'Table 1',
  tableFingerprint: 'fingerprint-1',
  rowIndex: 1,
  columnIndex: 2,
  columnHeader: 'Qty',
  oldValue: '1',
...
describe('useSteering', () => {
  ...
  describe('composer quotes + manual skill capture', () => {
    function setupWithContext(
      ...
      const rendered = renderHook(
        () => ({
          ...
          pendingComments: useRecoilValue(store.pendingMarkdownTableCommentsByConvoId(CONVO_ID)),
        }),
        { wrapper },
      );
      return { ...rendered, sendNow };
    ...
    const stageContext = ({ set }: MutableSnapshot) => {
      ...
      set(store.pendingMarkdownTableCommentsByConvoId(CONVO_ID), [markdownTableComment]);
    };

    it('queueFromComposer consumes staged quotes, skills, and table comments into the queued item', () => {
    ...
    it('leaves staged context untouched on the steer path (steers do not carry it)', () => {
      ...
      expect(result.current.pendingComments).toEqual([markdownTableComment]);
    });
    ...
    it('queues without quotes/skills fields when nothing is staged', () => {
      ...
      expect(result.current.queue[0].markdownTableComments).toBeUndefined();
    });
