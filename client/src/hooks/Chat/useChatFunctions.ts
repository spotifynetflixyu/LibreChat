...
export default function useChatFunctions({
  ...
  const ask: TAskFunction = (
    ...
    if (!!isSubmitting || (!isRegenerate && text.length > 0 && !isSubmittableMessage(text, (files?.size ?? 0) + replayFileCount))) {
      return false;
    }
    ...
    if (text.length === 0 && replayFileCount === 0 && (markdownTableComments?.length ?? 0) === 0) {
      return false;
    }
