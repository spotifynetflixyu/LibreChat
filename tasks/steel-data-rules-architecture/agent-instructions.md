...
3. AI uses it to interpret quote evidence, decide whether `lookup_quote_rules`
   is needed, choose reviewed lookup tools, and decide whether workbook output
   can be provisional or confirmed.
4. Detailed steel/material/process rules are not added here when they can be
   ...
   the merged `lookup_quote_rules` tool.
...
`toolRules`, `lookup_quote_rules`, and `search_price_candidates`. The text
...
`lookup_quote_rules = lookup_instructions + lookup_defaults`。AI runtime 只呼叫
merged `lookup_quote_rules`；不要把 `lookup_instructions` 或 `lookup_defaults`
當成可分開呼叫的 runtime tools。
...
公式預設、加工預設或公司計價習慣。不要另外呼叫 defaults-only runtime tool。若套用
customer-scoped default，必須向使用者說明。
