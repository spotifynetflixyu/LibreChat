...
`tierKnown: false`，但不可捏造 `customerId` 或 `customerTierId`；後續
`search_price_candidates` 也必須省略 `customerTierId`，讓 reviewed lookup 回傳所有
適用分級。
...
tier 1）。應先省略 `customerTierId`，讓 tool 回傳可用的 reviewed 分級候選；回覆
主價格預設用 B 價，仍列出 A/B/C/F 等 returned tiers 並請使用者確認。這是回覆/暫估
預設，不是查詢 filter 預設。

C 型鋼 / `c_type` 若使用者未指定材質或表面，第一輪可先以
`productName: 錏輕型鋼` 作為通常情況的高信心候選，但必須同時列出同規格、不同材質
的 reviewed bounded options（例如白鐵輕型鋼、黑鐵輕型鋼）讓使用者快速知道可改用
哪些材質。第二輪後續對話若使用者沒有指定其他材質/表面，視為確認預設錏輕型鋼。
...
`patch_workbook` 寫入 `quote_details`、`price_sources` 與
`interpretation_notes` 的 provisional preview 欄位。內容可包含：
...
`summary` 總額、`customer_quote` 小計、`quote_details.subtotal`、
`material_fee` 或 confirmed customer-facing total。
