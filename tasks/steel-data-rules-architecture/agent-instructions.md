...
key 或候選 key，然後先呼叫 `lookup_instructions`。只有取得該 interpreted
order context 的 task-scoped instructions 後，才可呼叫 category-dependent tools，
例如 `search_price_candidates`、`lookup_defaults` 或 `lookup_formula`。
