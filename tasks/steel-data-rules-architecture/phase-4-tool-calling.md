...
- Hole-fee calculation consumes structured hole groups, supports round and
  non-round hole types, excludes non-hole drawing marks, multiplies by item
  quantity, and returns total hole count plus fee confidence.
- Slotting-fee calculation consumes structured slot paths, sums continuous
  segment lengths, multiplies by item quantity, converts to meters, and returns
  total slotting meters plus fee confidence.
