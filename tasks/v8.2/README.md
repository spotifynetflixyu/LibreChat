# Steel v8.2 OpenHarness-Verified Development Plan

This folder converts [`docs/steel_librechat_plan_v8.2_openharness_verified.md`](../../docs/steel_librechat_plan_v8.2_openharness_verified.md) into an implementation roadmap with phase gates, checkpoints, and verification commands.
...
- Steel AI execution goes through `SteelAIProvider`; the orchestrator must not directly bind to OpenHarness or the official OpenAI client.
- `openharness_chatgpt_oauth` is the default local/dev driver. `openai_api` / `OPENAI_API_KEY` remains the capability fallback and production-safe fallback.
...
- OpenHarness session/conversation IDs are provider runtime trace only. Official OpenAI Responses/Conversations state is used only by the `openai_api` driver.
...
- Chinese source labels/headers/terms from `docs/reference/doc` must be mapped to English canonical schema keys before they shape DTOs, repository filters, SQL columns, tool arguments, workbook paths, prompt context, or mock API keys.
...
- Default `openharness_chatgpt_oauth` runtime for text/tool/structured workflows that pass smoke tests.
- `openai_api` fallback for official Responses API, production-safe operation, file/vision/XLSX evidence, hosted tools, rate-limit/cost observability, and any OpenHarness capability failure.
- Node/backend owns file upload metadata, provider file refs, tool execution, validation, audit, workbook truth, and fallback routing. AI owns primary interpretation and tool decisioning.
...
|---|---|---|
| 0 | [Decision Baseline](phase-0-decisions.md) | OpenHarness-verified v8.2 decisions are locked and stale OpenAI-only assumptions removed |
...
| 3 | [Quote Workbook MVP](phase-3-quote-workbook-mvp.md) | Chat UX sends messages and previews a seven-sheet workbook; ChatGPT OAuth binding is complete before OpenHarness live smoke; OpenHarness OAuth and OpenAI API fallback each have a live smoke case |
...
| OpenHarness ChatGPT/Codex OAuth Driver | Phase 3 |
| OpenAI API Fallback Driver | Phase 3 |
