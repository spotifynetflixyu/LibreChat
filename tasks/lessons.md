...
- In simplified Steel AI env examples, name the official OpenAI reroute switch by behavior: use `STEEL_OPENAI_OAUTH_AUTO_FALLBACK`, not `STEEL_OPENAI_API_ENABLED`, because the intent is automatic fallback from OAuth primary rather than globally enabling the secondary driver.
