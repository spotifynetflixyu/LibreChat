...
- [x] 以理論重量盤點可能受影響資料，確認六類長度規則與括號語意差異。
- [x] 新增六類6M預設、明確 M/L 長度、平鐵尺寸/單重與圓管括號外徑的失敗測試。
- [x] 實作共用長度 parser、平鐵 parser、圓管括號修正與平鐵預設黑鐵。
- [x] 品名含沒做、勿用、沒出、沒貨、不生產、不用、沒現貨時強制 `no_price` 並清空價格欄。
- [x] 更新平鐵、長條料與總查價 rules，補 rule sync tests。
- [x] 執行 focused tests、build、import/rules dry-run、workbook smoke 與 `git diff --check`。
- [x] 補上 Review；不得 apply DB。

Review:

- 共用 parser 現以 product_name 為長度真相：平鐵、角鐵、圓管、圓條、扁方管、方管名稱有 M/L 長度時優先解析，沒有標記時固定補 6000mm，不信任衝突的 workbook length 欄。本地 workbook 有 978 rows 的 length 輸出因此被補齊或修正（平鐵124、角鐵193、圓管245、圓條74、扁方管99、方管243）。
- 平鐵 parser 以來源品名解析寬、厚、括號整支重量與素材長度；英制寬 `5/8` 保留 nominal 並填 15.875mm。EIB05030 得到16×3、2.12kg/6M；EIB12160得到32×16、24.2kg/6M；EIS05020得到15.875×2、1.5kg/6M。括號重量 basis 固定 `kg_per_piece_or_stock_length`。
- 圓管括號語意已由單重修正為外徑：GOG1003 `3/8×2.3(17)` 得到 nominal 3/8、OD17、厚2.3、長6M，unitWeight 為空；不再把17誤當17kg。
- 平鐵 query 未提供或提供不合規 material 時由 tool 預設黑鐵；有效白鐵等 material 保持。
- importer 現對 product_name 的 `沒做|勿用|沒出|沒貨|不生產|不用|沒現貨` 做全類別 code-level 禁用，強制 `no_price` 並清空 direct/ratio 價格。Workbook reconciliation 由 confirmed 4880 / ratio_only 230 / no_price 1651，更新為4788 / 190 / 1783，共132筆原可報價狀態被正確禁用。
- 平鐵 rules 已確認並更新：預設黑鐵、keyword 只放寬×厚、括號單重/長度不入 keyword、明確素材長度優先、無長度6M、direct 支/只/片與 ratio-only 支轉Kg分流。六類6M通用契約同步寫入總查價與長條料規則。
- 驗證通過：focused Jest 6 suites / 134 tests、packages/api build、targeted ESLint、import dry-run 6,761 rows、rules dry-run、代表 workbook smoke、`git diff --check`。未執行 importer `--apply`、rules `--apply` 或任何 Supabase data update。

# Active: 圓管尺寸正規化與 ratio-only 支轉 Kg - 2026-07-13

Goal: 通用價格輸出將 `ratio_only + unit: 支` 視為 Kg tier price；圓管未指定材質預設黑鐵，並以來源 product_name 正規化直徑、厚度、長度與括號單位重，更新長條料規則，不同步 Supabase data 或 rules DB。

- [x] 盤點通用 ratio-only 輸出、圓管 live data、v4 parser 與長條料規則。
- [x] 新增 ratio-only 支轉 Kg、圓管 material default 與尺寸 parse 的失敗測試。
- [x] 實作通用有效 Kg unit、圓管預設材質與 importer parse。
- [x] 更新長條料/總查價規則與 rule sync tests。
- [x] 執行 focused tests、build、import/rules dry-run、workbook smoke 與 `git diff --check`。
- [x] 補上 Review；不得 apply DB。

Review:

- 通用 safe price output 現在將任何 `ratio_only + source unit: 支` 候選改成有效 `unit: Kg`，把 `tierRatios` 輸出成一般 `tierPrices`、`quoteUnit: Kg`；長條料因此採 weight billing，不再按整支 direct price 計價。`ratio_only + Kg` 與 M ratio 既有行為保持。
- 圓管 query 未提供或提供不合規 material 時由 tool 預設黑鐵；有效白鐵、錏、鋅等 material alias 保持原選擇。
- v4 importer 以 `product_name` 為準解析圓管公制/英制直徑、壁厚、第三段 M/L 長度與尾端括號單位重；`11/2` 正規化為 `1 1/2`，括號 `(17)` 轉成 `unitWeightValue: 17` 與 `kg_per_piece_or_stock_length`。不再信任 normalized text 中錯誤的 5.5in、t5900 等衍生值。
- `連料`、`太陽片` 只保留明確 3/4 管徑，不把 100x480 或溝數/代號解析成壁厚。規則同步限制一般圓管 query 不得帶入這些特殊品項。
- 長條料與總查價規則已加入圓管預設黑鐵、直徑×厚度 keyword、英制/公制二選一、括號單重與長度排除，以及 `ratio_only + 支` 轉 Kg 契約；舊 `4in 101.6mm` AND keyword 改為 `4in`。
- 本地 workbook smoke 驗證 FLV071006、GOB0004、GOB00607、GOG0004、GOG0005、GOG1003、GOG200150、JUU0948 的直徑、厚度、長度、英制與單重輸出。驗證通過：focused Jest 6 suites / 114 tests、packages/api build、targeted ESLint、import dry-run 6,761 rows、rules dry-run、`git diff --check`。
- 未執行 importer `--apply`、rules `--apply` 或任何 Supabase data update。

# Active: 圓條尺寸正規化與長條料切工規則 - 2026-07-12

Goal: 修正本地 v4 importer 對圓條裸公制直徑、英制分數與素材長度的解析，清除來源名稱未提供的錯誤英制推論；將長管切工規則改名為長條料切工，補充圓鐵專屬與管類通用規則，不更新 Supabase data 或 rules DB。

- [x] 盤點圓條 live data、v4 parser、rule sync metadata 與舊檔名引用。
- [x] 先新增圓條尺寸解析與錯誤英制推論的失敗測試。
- [x] 實作最小 importer parse 修正。
- [x] 將規則檔改名為 `長條料-切工.txt`，更新 sync metadata/tests，補充圓鐵專屬與管類通用規則。
- [x] 執行 focused tests、import dry-run、rules dry-run 與 `git diff --check`；不得 apply DB。
- [x] 補上 Review。

Review:

- v4 importer 現在從圓條來源名稱正規化明確 `mm/m/m`、裸小數公制直徑、`x3M`/`(6M)` 素材長度，以及 `1/4`、`3/8`、`11/4`、`13/4` 等英制尺寸；英制同時保留 canonical `nominalInch` 並填精確 `outerDiameterMm`，query keyword 仍依規則保留原英制分數。
- 純公制圓條不再信任 workbook 自動推論的管類 inch alias；例如 EQA0381 已由錯誤 `5.5in/OD139.7` 修正為 `od38`、38mm、無 nominal inch。`12mm-50mm切` 等範圍列不再被壓成單一直徑。
- 本地 workbook smoke 驗證 EQA00635、EQA00801、EQA0127、EQA0318、EQA0381、EQA0406、EQS0030 的直徑、英制與長度輸出符合來源名稱。
- 規則檔已改名為 `docs/rules/類別規則/長條料-切工.txt`；rule slug 保持 `steel_quote_rules_long_material_cutting`，舊類別規則路徑加入 source-ref cleanup。規則新增管類通用 query、圓鐵預設黑鐵、直徑 keyword 正規化、長度分離，以及一般圓鐵/圓條 → 中碳 → 磨光中碳的候選順序。
- 驗證通過：focused Jest 3 suites / 56 tests、packages/api build、targeted ESLint（忽略該測試檔既有 CommonJS require 規則）、import dry-run 6,761 rows、rules dry-run、`git diff --check`。未執行 importer `--apply`、rules `--apply` 或任何 Supabase data update。

# Active: 通用 keyword 禁止專有品名 - 2026-07-12

Goal: 所有價格類別首次查價只使用通用產品名稱與規格，移除 rules 中自行推導的來源／專有品名 keyword；只有用戶原文明確指定時才可保留。

- [x] 盤點所有 docs/rules keyword 指引、JSON 範例與來源品名 token。
- [x] 先新增通用 keyword 契約與禁止舊專有範例的失敗測試。
- [x] 更新 agent、總查價、長管、孔及所有受影響類別規則／範例。
- [x] 執行 focused tests、local rule manifest dry-run、live generic lookup smoke 與 `git diff --check`。
- [x] 補上 Review；不得同步 rules DB 或更新 Supabase data。

Review:

- Agent 與總查價契約現在明定：首次 keyword 只能用通用產品名稱與規格；禁止自行加入專有品名、廠牌、系列、來源 DB token 或代號。只有用戶原文明確指定時才可原樣保留，候選 product_name 不得反向成為修正 query。
- 所有 rules JSON keyword 範例已移除來源／專有 token；進一步移除已由 category/subcategory 表達的平鐵、角鐵、方管、扁方管、圓鐵、方鐵、槽鐵、工字鐵、鋼管等類別詞，keyword 只保留 `50x6mm`、`100x6`、`40x80x3`、`10mm`、`25mm`、`200x90x8/13.5`、`4in 101.6mm` 等規格。
- 加工、其他、捲門、格板、五金、門窗、板/浪板、孔等 query 指引同步限制為通用產品／加工名稱；型號、廠牌或專有名稱只有用戶明確指定時才可使用。
- Live grouped smoke 的 8 個 spec-only keyword 全部 status=ok：10mm 1/1 可報價、25mm 6/4、平鐵 category + 50x6mm 2/2、角鐵 category + 50x6mm 5/5、200x90x8/13.5 4/4、4in 101.6mm 2/2、100x6 1/1、40x80x3 2/2。
- 驗證通過：focused Jest 5 suites / 82 tests、local rule manifest dry-run、禁用 keyword 全 rules 掃描零命中、`git diff --check`。未同步 rules DB，未更新 Supabase data。

# Active: 鐵板容錯查價與 thickness parse - 2026-07-12

Goal: 鐵板 query 由代碼補 unit/material 預設、全類別容錯正規化材質別名、整片候選保留給 AI 配料，並把 Kg ratio-only 輸出成一般價格格式；同時修正 2B/NO1 數字污染板厚的 importer parser，不更新 Supabase data 或 rules DB。

- [x] 盤點 tool schema、repository SQL、safe result、鐵板 live rows 與 v4 thickness parser。
- [x] 先新增 input defaults/aliases、unit filter、ratio-only output、板厚 parser 的失敗測試。
- [x] 實作最小 schema/repository/result/parser 改動與鐵板本地 rules。
- [x] 驗證 unit=片 不使用尺寸 filter、候選完整保留，以及預設加工價格排序。
- [x] 執行 focused tests、coverage、build、local parse smoke 與 `git diff --check`。
- [x] 補上 Review；不得重匯或更新 Supabase data，不得同步 rules DB。

Review:

- 鐵板 query 缺少 unit/material 時由 tool 補 `Kg`/`黑鐵`；鐵板 unit 的 Kg/kg 正規化為 Kg，其他值轉片。其他類別保留 AI 傳入的 unit 原值並套用 repository unit filter。
- material input 改為容錯正規化且適用所有類別：支援白鐵/ST/不鏽鋼、2B/NO1/HL/BA、沙面/亮面及 DB 風格完整 material 字串；不合規值不拋錯，鐵板回退黑鐵，其他類別省略 material filter。鐵板泛稱白鐵/ST 在厚度全小於 3mm 時轉 2B、全大於等於 3mm 時轉 NO1。
- 鐵板未指定加工 keyword 時，results 依雷射切割、四方切、版型切型排序；明確 keyword 保留 repository 篩選結果。`unit: 片` 不新增 sheet dimensions filter，8 筆尺寸候選完整返回 AI 配料。
- 所有類別的 `ratio_only + unit: Kg` 都輸出成一般 `tier_price` / `tierPrices`；M ratio 維持既有 ratio option。
- v4.3 parser 會從鐵板 product_name 取得真實厚度，修正 2B 的 2、NO1 的 1 污染；只有 ST2B/STNO1 而無真實厚度的泛稱列改為 null。本地 workbook smoke 已確認 BNS030408/410/510 為 3mm、DNT700100/DNT900100 為 10mm、B4NS900010 為 1mm、BNS 泛稱為 null。
- 驗證通過：focused Jest 5 suites / 81 tests；目標檔 statements 92%、branches 85.25%、functions 97.52%、lines 91.81%；packages/api build、targeted ESLint、import dry-run 6,761 rows、`git diff --check`。
- 沒有執行 importer `--apply`、Supabase data update、rule dry-run/apply 或 DB readback。

# Active: H型鋼 stockLengthMm 查價 - 2026-07-12

Goal: 新增 H 型鋼母材長度 OR filter，讓 AI 先完成常規／非常規配料規劃，再只查實際會使用的價格長度，同時由 backend 固定排除 no-price rows。

- [x] 確認 `stockLengthMm` 採 array OR 語意，DB 對應 `steel.prices.length_mm`。
- [x] 先新增 tool schema、repository serialization/SQL 與 no-price 排除的失敗測試。
- [x] 實作最小 tool/repository 改動並更新 H型鋼本地 rules。
- [x] 執行 focused tests、coverage、build 與 `git diff --check`。
- [x] 補上 Review；本輪不得同步 rules DB。

Review:

- `search_price_candidates` lookup 新增 `stockLengthMm: string[]`，repository 以 `steel.prices.length_mm` 精確 OR 比對；輸入正規化會去重、統一 decimal 字串並最多保留 20 筆。
- `stockLengthMm` 中格式錯誤、非字串、非正數或科學記號等不合規元素會靜默移除，不因單一壞值拒絕整筆 query；H 型鋼再額外移除 `< 6000mm` 的值，6000mm 保留。
- lookup 與 category discovery 都由 SQL 固定排除 `value_state = 'no_price'`，此條件未寫入 AI rules。
- H型鋼 rules 已改為斷面 keyword + `stockLengthMm` 配料查詢，常規母材為 6M、9M、10M、12M；Kg 材料按成品實際重量計價，切工另外比較。
- TDD red phase 確認舊 schema/SQL 會失敗；green phase focused Jest 3 suites / 30 tests 通過。目標檔 coverage：statements/lines 85.85%，functions 92.3%。packages/api build 與 `git diff --check` 通過。
- live `.env` smoke 將 `[4000,5000,6000,9000]` 正規化成 `[6000,9000]`，只返回 EHS101006 6M 與 EHS101009 9M confirmed rows。依使用者指示未同步 rules DB。

# Active: C型鋼初查 keyword 與 C2／2C 規則 - 2026-07-12

Goal: 依確認內容更新 C型鋼初查規則，讓一般 C 型鋼預設錏材質、keyword 排除長度、樑柱只查通用名稱，並統一 C2／2C 結筒表記。

- [x] 核對現有 C型鋼專屬規則、總查價規則與 live `steel.prices` 命名。
- [x] 更新 C型鋼專屬規則與總查價規則，不改查價程式。
- [x] 驗證 C2、2C 都只在兩支結筒時附加 `型鋼結筒加工費` query。
- [x] 執行本地 focused rule tests 與 `git diff --check`；本輪不得同步 DB，待所有類別完成後統一處理。
- [x] 補上 Review 結果。

Review:

- C型鋼未指定材質時首查固定使用 `material: "錏"`；一般 keyword 只保留明確截面與厚度，長度只參與 Kg 重量計算。
- 樑柱 keyword 固定只使用 `樑柱`；沒有厚度時由其他條件相符且可報價的候選採最薄厚度。
- `C2`、`2C` 統一表示兩支 C 型鋼結筒，只有這兩種明確表記才附加 `型鋼結筒加工費` query，一般單支禁止查此加工費。
- 本地 `sync-steel-rules.spec.ts` 通過，相關檔案 `git diff --check` 通過。依使用者指示未執行 rule dry-run、DB apply 或 DB readback；待所有類別完成後一次同步。
...
- [x] Verified: - `api`: `rtk npx jest server/services/MCP.spec.js --runInBand` - `packages/api`: `rtk npx jest src/steel/vision/ocr.spec.ts --runInBand --coverage=false` - root: `rtk git diff --check`
...
- [x] Production S3 OCR smoke passed with private bucket: - Uploaded `/data/smoke/b.png` to temporary S3 key under `uploads/smoke/`. - Generated a presigned URL with `X-Amz-Expires=43200`. - Called `paddleocr_vl` from the production container using that URL. - OCR returned `2664` chars in `17774` ms. - Deleted the temporary S3 object.
...
# Active: v4.3 production deployment and sync - 2026-07-12

Goal: deploy the committed v4.3 pricing/runtime changes through the canonical
`master` production workflow, then migrate and synchronize production
`steel.prices` and reviewed `steel.rules` using `.env.prod`, with live runtime
and database readback.

- [x] Confirm the exact commit range from `origin/master` to the deployment HEAD and keep the worktree clean.
- [x] Run deployment-gate focused tests, package build, importer/rule production dry-runs, and diff hygiene.
- [x] Fast-forward `master` to the reviewed deployment HEAD and monitor the production workflow to completion.
- [x] Verify public `/health`, `/readyz`, and `/api/config` build commit metadata.
- [x] Apply pending v4.3 migrations to production Supabase through `.env.prod.admin`.
- [x] Upsert all 6,761 v4.3 prices by `erp_item_code` and apply reviewed rules to production.
- [x] Read back production dataset/category/state counts, representative parsed rows, rule hashes, migration versions, and advisors.
- [x] Record the final production review here; do not alter host env/config unless current evidence requires it.

Deployment scope:

- Production workflow: `.github/workflows/deploy-prod.yml`, triggered by a push to `master`.
- Current deployment candidate: `feat/v8.5` at `e73f9744d53b584433075cfaae47ffb96dffbfcf`;
  `origin/master` is `aa88ef5b90208d3fe35ca54bc1c51328b188420a`.
- Production DB/rule commands must load `.env.prod`; dev `.env` is not valid evidence for this rollout.
- Production DDL migrations/admin link use `.env.prod.admin` `PROD_ADMIN_URL`; price/rule apply and runtime readback use `.env.prod` `STEEL_POSTGRES_URL` (`prod_app`).

Review:

- Production admin preflight proved `.env.prod.admin` `PROD_ADMIN_URL` connects as
  `postgres`; `.env.prod` runtime connects as `prod_app.qqgajxkztktvifnyldgj`.
  No host env or `/data/librechat.yaml` change was required.
- Applied expansion migration `20260712080613`, upserted v4.3 through runtime
  `prod_app`, then applied canonical migration `20260712081224`; both versions and
  names are present in `supabase_migrations.schema_migrations`.
- Production `steel.prices` readback: 6,761 total / 6,761 distinct ERP codes,
  4,880 confirmed, 230 ratio-only, 1,651 no-price, 85 `圓條`, 2 `鋼筋`,
  0 `圓鐵`, and every row uses `product_price_v4_3`. Final DB constraints allow
  only v4.3 source rows and the new canonical category enum.
- Representative parsed-row readback passed for DZA0345, ELH020, KADS06,
  BKZZM2, BLZZM3, DNB2002, BKZA010, EQC0280, EQD0090, and EQD0120.
- Production reviewed-rule SHA readback matches repo: agent `32111b13...e2e5e`,
  hole `8d8c3760...47d52`, long-material cutting `28c12e34...b29c`, lookup guide
  `250f697b...aea7`.
- Pushed `e73f9744d53b584433075cfaae47ffb96dffbfcf` to `master`. GitHub Actions
  production run `29189156207` completed successfully in 4m18s; image build/push,
  Droplet deploy, container health, and public URL smoke all passed.
- Live `https://chat.longdin.org/health` and `/readyz` return `OK`.
  `/api/config` reports commit `e73f9744d53b584433075cfaae47ffb96dffbfcf`,
  branch `master`, build date `2026-07-12T10:27:58Z`.
- Deployment gates: focused Jest 8 suites / 121 tests, packages/api build,
  production importer/rule dry-runs, committed-range `git diff --check`, final
  runtime and DB readbacks all passed.
- Production advisors show only pre-existing mutable-search-path warnings for
  `steel.record_price_history` / `steel.set_updated_at` and public `pg_trgm`;
  this rollout introduced no new advisor warning.
...
# Active: 方鐵長度與圓管括號重量修正 - 2026-07-13

Goal: 方鐵只解析品名明示長度，不套用無長度 6M 預設；B 管尾端括號數字解析為 6M 每支重量。

- [x] 修正 parser 長度與單重語意，保留六類長條料的既有 6M 預設。
- [x] 更新方鐵與圓管 regression tests，鎖定明示長度、缺省長度及 B 管括號重量。
- [x] 更新本地方鐵 rules 與 rules regression；不同步 Supabase。
- [x] 執行 focused Jest、build、dry-run、lint 與 diff hygiene。
- [x] 補上 review 與驗證結果。

Review:

- `鍍鋅B管 3/8*2.3(17)` 保留 NPS 3/8、壁厚 2.3mm，解析為 `lengthMm: 6000`、`unitWeightValue: 17` 與整支重量 basis。
- 方鐵只接受品名明示的 M/L 長度：`1/4 (3M)` 為 3000mm、`6.0*6M(1.72)` 為 6000mm；`10.0(4.79)` 與切割級距保持 `lengthMm: null`，不使用來源欄位或6M預設補值。
- 方鐵本地 rules 已固定預設黑鐵、keyword 只放邊長、一般黑鐵優先於磨光，以及無明示長度不得把括號單重視為6M重量。
- 驗證通過：focused Jest 4 suites / 133 tests、packages/api build、相關 TypeScript ESLint、價格 importer dry-run（6,761 rows；4,788 confirmed、190 ratio_only、1,783 no_price）、rules dry-run、`git diff --check`。未同步 Supabase data/rules。

# Active: 方管 parser 與 keyword 規則 - 2026-07-13

Goal: 一般方管正規化邊長、壁厚與素材長度；特殊太陽片、雨棚、沖孔窗、連料不得把成品尺寸污染成材料厚度。

- [x] 新增方管專用 parser，支援公制、英制、正方尺寸與明示 M/L 長度，無長度固定6M。
- [x] 分離一般材料與特殊成品解析，避免尺／寸、U型尺寸、沖孔窗雙尺寸污染厚度。
- [x] 新增 workbook 命名 regression tests，確認 canonical dimensions 與特殊品 fallback。
- [x] 更新本地方管 keyword/rules 與 rules regression；不同步 Supabase。
- [x] 執行 focused Jest、build、import/rules dry-run、lint 與 diff hygiene，補上 review。

Review:

- 一般方管現在從 product_name 解析公制、英制分數／引號、`正40`、壁厚及明示 M/L 長度；`25mm*1.2` 保留25mm而非換成25.4mm，`200x6x12M` 為邊長200、厚6、長12000，未標長度仍為6000。
- 太陽片、雨棚、沖孔窗、連料、U型先進特殊成品路徑，只保留安全的名義方管邊長；2尺5、雙內80、110x480等成品變體不再污染壁厚。對 workbook 243筆方管重播後，158筆一般料中154筆完整解析，剩餘4筆是本來就沒有尺寸的泛稱品名。
- 本地 rules 已固定一般料 keyword 為單一邊長x壁厚、BA只用 material、素材長度不進 keyword；特殊成品則使用通用品種、名義邊長與必要成品尺寸，連料帶 `subcategory: "連料"`。
- 驗證通過：focused Jest 4 suites / 150 tests、packages/api build、相關 TypeScript ESLint、價格 importer dry-run 6,761 rows、rules dry-run、`git diff --check`。未同步 Supabase data/rules。

# Active: 扁方管 parser、rules 與無生產標記 - 2026-07-13

Goal: 扁方管正規化寬x高x壁厚與6M素材長度，並將品名「無生產」統一轉為 no_price。

- [x] 稽核 workbook 扁方管欄位與現有 parser 語意。
- [x] 新增扁方管專用 parser 與 regression tests，支援寬x高x厚、明示 M/L 及無長度6M。
- [x] 將「無生產」加入品名 no_price code path 並補測試。
- [x] 更新本地扁方管 rules 與 rules regression；不同步 Supabase。
- [x] 執行 focused Jest、build、import/rules dry-run、lint、workbook replay 與 diff hygiene，補上 review。

Review:

- 扁方管 parser 以 product_name 正規化寬x高x壁厚，支援 `*`/`x` 與分隔符後空格；明示 M/L 長度優先，未標長度固定6000mm。`10x20x6M` 等舊品名只解析寬10、高20、長6000，厚度保持 null，不再採 workbook 污染的 t6。
- Workbook replay：99筆扁方管全部長度6000；96筆有尺寸，其中92筆一般料完整解析壁厚，4筆 `寬x高x6M` 厚度全部為null，3筆無尺寸泛稱保持null。
- `無生產` 已加入 importer code marker；實際 `白鐵扁方管 40*80*1.2(無生產)` 改為 no_price 並清空 direct/ratio 價格。匯入 reconciliation 更新為4,787 confirmed、190 ratio_only、1,784 no_price。
- 本地 rules 已固定未指定材質預設黑鐵、keyword只放寬x高x壁厚、素材長度不進keyword，並明載 `寬x高x6M` 不代表壁厚。
- 驗證通過：focused Jest 4 suites / 157 tests、packages/api build、相關 TypeScript ESLint、價格 importer dry-run 6,761 rows、rules dry-run、workbook replay、`git diff --check`。未同步 Supabase data/rules。

# Active: 槽鐵 parser、6M預設與熱浸鍍材質 - 2026-07-13

Goal: 槽鐵解析完整斷面、素材長度與括號整支重量；未標長度預設6M，熱浸鍍統一為錏／鍍鋅。

- [x] 新增槽鐵專用 parser 與6M預設，分辨腹厚/翼厚、長度與括號重量。
- [x] 全類別品名「熱浸鍍」材質正規化為錏／鍍鋅並補測試。
- [x] 更新通用與槽鐵本地 rules：預設6M材料不固定用 stockLengthMm 篩選，成品長度不作query。
- [x] 重播槽鐵 workbook 並執行 focused Jest、build、dry-run、lint、diff hygiene；不同步 Supabase。
- [x] 補上 review。

Review:

- 槽鐵 parser 現在解析高度、翼寬、腹厚/翼厚、明示M/L長度與長度後括號整支kg；品名無長度固定6000mm。`50x25x5x6M(22)` 為長6000、整支22kg，`75x40x5/7` 保留腹厚5/翼厚7，`200x80x12M(310)` 為長12000、厚度未知、整支310kg。
- 全類別 product_name 含熱浸鍍時 importer material 統一為 `錏 / 鍍鋅`；query schema 將熱浸鍍/熱浸鍍鋅正規化為 `material: 錏`，槽鐵未指定材質預設黑鐵。
- 通用rules已明載七類預設6M材料（含槽鐵）的query不帶stockLengthMm，AI從返回的6000、12000或其他可用母材做不切清配料；訂單成品長度不作母材query filter。全rules沒有單一6000的stockLengthMm JSON範例；H型鋼多母材array策略保留。
- Workbook replay 136筆槽鐵：熱浸鍍材質全部為錏/鍍鋅；白鐵無長度品名補6000；代表性明示長度、單重與厚度語意均通過。
- 驗證通過：focused Jest 4 suites / 163 tests、packages/api build、相關 TypeScript ESLint、價格 importer dry-run 6,761 rows（4,787 confirmed、190 ratio_only、1,784 no_price）、rules dry-run、workbook replay、`git diff --check`。未同步 Supabase data/rules。

# Active: 角鐵 parser、尺長度與 keyword rules - 2026-07-13

Goal: 正確解析等邊/不等邊/成型/烤漆角鐵、素材重量與長度，配件不套6M；尺/inch/mm換算採整數毫米比對。

- [x] 新增角鐵專用 parser，修正BA/2B厚度、成型角鐵、括號重量與不等邊尺寸。
- [x] 明示尺/M/L長度優先、未標材料預設6M；角鐵配件不補6M。
- [x] 熱進鍍鋅 typo 也正規化為錏／鍍鋅；角鐵未指定材質預設黑鐵。
- [x] 更新本地角鐵與通用單位 rules、rules regression；不同步 Supabase。
- [x] 執行 workbook replay、focused Jest、build、dry-run、lint、diff hygiene並補review。

Review:

- 角鐵 parser 現在區分等邊、不等邊與烤漆萬能角鋼；BA/2B 25x2不再誤為t25，錏成型25x2.5x6M不再把6M當厚度，括號數字優先作該素材長度整支kg重量。
- 一般角鐵明示M/L/尺長度優先，未標固定6000mm；烤漆12尺保留原文並以台制1尺303mm正規化為3636mm。塑膠腳套左右配件保持lengthMm null，不套6M。
- 熱浸鍍與來源常見typo熱進鍍鋅都正規化為錏/鍍鋅；query輸入同樣轉material錏，角鐵未指定材質預設黑鐵。
- 通用規則已明載尺、inch、mm或其他長度單位保留原文，轉mm供query/候選比對時四捨五入為整數。stockLengthMm code也統一整數化並去重，不使用小數位比對。
- Workbook replay 193筆角鐵；代表性BA、不等邊、8M、進口、烤漆12尺、成型、熱進鍍鋅、2B與配件語意均通過。
- 驗證通過：focused Jest 4 suites / 174 tests、packages/api build、相關TypeScript ESLint、價格importer dry-run 6,761 rows（4,787 confirmed、190 ratio_only、1,784 no_price）、rules dry-run、workbook replay、`git diff --check`。未同步Supabase data/rules。

# Active: 網 parser、unit計價與 keyword rules - 2026-07-13

Goal: 網類依次類別解析線徑、孔目、片/捲尺寸與長度，修正ST304/浪型網污染；以category/subcategory/material取得候選交給AI判斷，不用unit filter且維持通用limit 30。

- [x] 新增網類parser與subcategory修正，覆蓋ST304厚度、浪型網、片/捲尺寸、刺網長度重量，以及點焊網包裝數不得回退成單重。
- [x] 網類unit alias正規化；㎡/才按面積direct，丸/片/只按整件direct，丸不足一捲仍按整捲。
- [x] 網類query不使用keyword、unit或額外尺寸params；省略limit時維持通用預設30，將符合category/subcategory/material的候選交給AI判斷。
- [x] 新增 `docs/rules/類別規則/網.txt`，集中保存網類parser、query與計價規則；保留查價總表的類別摘要。
- [x] 新增parser/schema/rules regression與workbook replay；不同步Supabase。
- [x] 執行focused Jest、build、dry-run、lint、diff hygiene並補review。

Review:

- 網類首查固定不用keyword、unit或額外尺寸params；省略limit時維持通用預設30，讓AI從符合category/subcategory/material的候選挑足夠裁切且較划算者，維持不切清。
- 新增獨立 `網.txt` 與sync metadata；查價總表仍保留完整網類摘要。未同步Supabase data/rules。
- parser可辨識點焊網5.5／6.0足／8.0線徑、15x15cm孔與2Mx3M片材；尾端(6)及污染的來源單重均清為null。刺網105M(8.2KG)保留8.2kg整捲重，ST304固定片不產生304mm厚度。
- Workbook replay共60筆網類；代表點焊網、刺網與固定片結果符合預期。
- 驗證通過：focused Jest 4 suites / 171 tests、packages/api build、targeted ESLint、價格importer dry-run 6,761 rows（4,787 confirmed、190 ratio_only、1,784 no_price）、rules dry-run與workbook replay。

# Active: 方鐵獨立類別規則 - 2026-07-13

Goal: 方鐵不屬於長條料，將其query、長度、單重與計價契約移至獨立 `方鐵.txt`，避免長條料通則誤套。

- [x] 新增 `docs/rules/類別規則/方鐵.txt` 與rule sync metadata。
- [x] 從 `長條料-切工.txt` 移除方鐵適用範圍、查價範例及專屬段落。
- [x] 新增rules regression，確認方鐵規則只在獨立文件且未同步Supabase。
- [x] 執行focused Jest、rules dry-run、lint及diff hygiene並補review。

Review:

- 方鐵query、磨光採價順序、明示長度、括號單重、Kg優先與切工人工複核規則已集中到獨立 `方鐵.txt`。
- `長條料-切工.txt` 已移除方鐵適用範圍、查價範例與專屬段落；方鐵邊界只保留在獨立 `方鐵.txt`，避免重複規則。
- 同輪依更正移除網類unit query filter與limit 100 override；網未指定limit時維持通用30。
- 驗證通過：focused Jest 4 suites / 172 tests、packages/api build、targeted ESLint、rules dry-run與 `git diff --check`。未同步Supabase data/rules。

# Active: 鋼筋、鐵軌 parser 與 rules 稽核 - 2026-07-13

Goal: 唯讀檢查鋼筋與鐵軌的實際workbook命名、parser輸出、query規則及測試覆蓋，先整理缺口，不修改parser/rules。

- [x] 盤點鋼筋來源資料、現行parser、代表輸出與rules。
- [x] 盤點鐵軌來源資料、現行parser、代表輸出與rules。
- [x] 比對測試覆蓋、查價語意與長度／重量欄位污染。
- [x] 整理建議修正範圍，等待用戶確認後再實作。

Review:

- 鋼筋只有9mm(3#)、12mm(4#)兩筆且全為no_price；現行parser未保存直徑／號數，但沒有誤補6M。material空白，首查不應加黑鐵；目前必然no_match並送人工複核。
- 鐵軌共10筆，5筆confirmed、5筆no_price；6K/9K為6M，12K/15K/22K等為10M，不能全類別預設6M。現行parser沒有K級別canonical欄位。
- `6K鐵軌 6M(38)(件55支)` 的來源unitWeightValue=36與品名、價格計算矛盾，應由專屬parser以第一個純數字括號38覆蓋；件55支不是重量。
- 鐵軌keyword只用canonical K級別；明確母材長度應用stockLengthMm而不是塞入keyword。鋼筋與鐵軌目前都缺少parser regression。
- 本輪只做唯讀稽核與紀錄，未修改parser/rules，也未同步Supabase。

# Active: 鋼筋、鐵軌 parser 與 rules 實作 - 2026-07-13

Goal: 依稽核結果正規化鋼筋直徑／號數與鐵軌K級別／明示長度／整支重量，修正query及人工複核規則。

- [x] 新增鋼筋parser：9m/m、12m/m轉直徑，3#、4#保留為規格，不誤作重量或長度。
- [x] 新增鐵軌parser：canonical K級別、明示M長度、第一個純數字括號整支重量，排除包裝與凸面數字。
- [x] 更新查價方式與長條料rules：鋼筋不加material/不補6M；鐵軌長度用stockLengthMm且不全類預設6M。
- [x] 新增parser/rules regressions並重播全部鋼筋、鐵軌workbook rows。
- [x] 執行focused Jest、build、lint、import/rules dry-run與diff hygiene；不同步Supabase。

Review:

- 鋼筋replay：9m/m(3#)轉 `od9|rebar3`，12m/m(4#)轉 `od12|rebar4`；直徑分別9/12mm，長度、厚度、單重保持null。兩筆仍為no_price。
- 鐵軌replay：6 K正規化rail6k；6K/9K保留6000mm，12K以上依品名保留10000mm，特殊5.5M保留5500mm且因勿用轉no_price。
- `6K鐵軌 6M(38)(件55支)` 現以38kg覆蓋污染的36kg；件55支與凸面22.0/25.4不污染重量或厚度。
- rules改為鋼筋query不帶material/stockLengthMm且no_price後人工複核；鐵軌keyword只用K級別，明確母材長度才用stockLengthMm，不把長度放keyword。
- 驗證通過：focused Jest 4 suites / 179 tests、packages/api build、targeted ESLint、import dry-run 6,761 rows、rules dry-run、workbook replay與 `git diff --check`。未同步Supabase data/rules。

# Active: 方鐵Kg與density計價修正 - 2026-07-13

Goal: 方鐵比照C型鋼只採Kg價，使用實心方形截面與candidate density計算成品重量。

- [x] tool固定方鐵query `unit: Kg`，傳入其他unit也正規化為Kg。
- [x] 更新方鐵獨立規則與查價總表，移除支價fallback。
- [x] 明載重量公式與candidate density要求，補schema/rules regression。
- [x] 驗證通過：focused Jest 2 suites / 41 tests、packages/api build、targeted ESLint、rules dry-run與diff hygiene；未同步Supabase。

Correction:

- 方鐵同時有可用Kg與支價格，已移除tool的unit=Kg預設與rules中的類別類比。首查不加unit filter；Kg以截面×長度×candidate density計重，支以明示整支長度配料並按direct支價。
...
# Active: 本地 Steel rules 分層去重 - 2026-07-13

Goal: 統一檢查並精簡全部 `docs/rules`，讓通用規則只存在於
`agent規則.txt`、`輸出規則.txt`、`查價方式.txt`，長條料與切工各自集中，
細項規則只保留類別／OCR 專屬差異，降低重複 context tokens；本輪只修改本地
rules 與同步程式／測試，不 apply rules DB。

- [x] 讀取專案指示、lessons、rules 載入順序與所有本地 rule files。
- [x] 盤點通用、長條料、切工、類別與 OCR 規則的重複及責任邊界。
- [x] 精簡 `agent規則.txt`、`輸出規則.txt` 與 `查價方式.txt`。
- [x] 將 `長條料-切工.txt` 拆為 `長條料.txt` 與 `切工.txt`，更新 sync metadata／cleanup。
- [x] 精簡類別／其他細項規則，移除已由通用規則涵蓋的重述。
- [x] 更新 focused regression tests，加入跨檔重複與 rule source 完整性檢查。
- [x] 執行 focused Jest、rules dry-run、重複掃描、token/字數比較與 `git diff --check`。
- [x] 補上 Review；不得 apply rules DB。

Review:

- Rules 已分成唯一 owner：agent 只管角色／工具 routing／資料優先序；輸出規則保留四張 sheets 與 16 欄契約；查價方式只保留 lookup／選價通則及 registry index；長條料集中配料與各長料差異；切工只保留 candidate mapping、刀數、加價與限制；加工及其他產品各有細項檔。
- 解決三個原有衝突：C2／2C 結筒 query 只以 `C型鋼.txt` 為準；方鐵定位為實心方管型長條料並完整套用長條料通則，query／實心Kg計重／不補6M等例外留在細項；同一 OCR file key 只允許一張 final 表，以 `OCR規則.txt` 為準。
- 本地 rules 由11檔／41,031 Unicode characters 改為14檔／19,791 characters；雖增加清楚的 owner files，實際 context 字元減少21,240（51.8%）。新增 sentence-level regression，28字以上跨檔完全重複片段為0。
...
# Active: Usage remaining partial payload tolerance - 2026-07-13

Goal: OpenAI OAuth Usage remaining 對 upstream 部分欄位或 window 缺失採容錯解析；
只顯示實際可用資料（例如只有 weekly），不得因非必要 key 缺失讓整個 Usage fail。

- [x] 以目前 live WHAM response shape 與 partial fixtures 重現 parser/UI 行為。
- [x] 調整 shared usage schema 與 backend parser，逐欄位保留可用資料並略過空 window。
- [x] 調整 UI，缺少 label/reset/percent 資料時只隱藏該部分。
- [x] 新增 backend 與 client regression tests，涵蓋 weekly-only 與 window partial fields。
- [x] 執行 focused tests、build/typecheck、targeted ESLint 與 `git diff --check`。
- [x] 補上 Review，記錄 live shape、root cause 與驗證結果。

Review:

- Live 去識別化 shape 檢查確認頂層仍使用 `rate_limit`；目前帳號的 primary window
  是 `604800` 秒 weekly、secondary window 為 `null`，證明 window key 不是固定週期且
  upstream 會讓個別 window 缺資料，
  但沒有證據需要支援臆測的頂層 alias。
- Root cause 是 backend 要求每個 window 同時具備 `used_percent`、duration、
  reset-after 與 reset-at；任一非必要 metadata 缺失就丟掉整個 window，全部被丟掉時
  才轉成 `invalid_response`。Shared schema 也把所有 metadata 誤設為 required。
- Shared schema 與 parser 現只以可計算 remaining 的 `used_percent` 作 window 存活條件；
  duration、reset-after、reset-at 改為 optional 並只保留有效存在值。空或缺少 percent 的
  window 個別略過，其他可用 window（包含 weekly-only）繼續回 `available`。
- UI 對 duration label 與 reset label 採 conditional rendering；secondary 即使沒有
  duration 仍以 key 顯示 Weekly，primary 缺 duration 不再產生 `NaNm`，缺 reset 不留空節點。
...
- [x] 以【search_price_candidates 查價規則】作唯一詳細 owner，類別細則只保留例外與 fallback。
