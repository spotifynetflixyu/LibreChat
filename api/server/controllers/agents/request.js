...
const { saveConvo, saveMessage, getMessages, getConvo, isAgentTriggerPrincipalActive } = require('~/models');
const {
...
function getPreliminaryUserMessage(
  ...
  return {
    ...
    // Persist uploaded files on this awaited preliminary write so they land on
    // job.metadata.userMessage before the run can reach its first interrupt.
    ...(preliminaryFiles !== undefined && { files: preliminaryFiles }),
...
const ResumableAgentController = async (req, res, next, initializeClient, addTitle) => {
  ...
  try {
    ...
    if (
      isNewConvo &&
      !recoveredSteerId &&
      Array.isArray(preliminaryUserMessage?.files) &&
      preliminaryUserMessage.files.length > 0
    ) {
    ...
    const startGeneration = async () => {
      ...
      try {
        ...
        const shouldGenerateTitleAfterFinal = shouldGenerateTitleAfterTurn({
          ...
          title: conversation.title,
          isTemporary: req.body?.isTemporary,
          wasAborted: wasAbortedBeforeComplete || preemptIncomplete,
        ...
        } else if (shouldGenerateTitleAfterFinal || shouldGenerateTitle) {
