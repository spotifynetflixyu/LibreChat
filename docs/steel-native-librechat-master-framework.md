...
9. LibreChat file messages and attachments stay owned by LibreChat file records and permissions. Native provider vision/file inputs must still be available to the AI agent for drawing reasoning such as holes, bends, slots, and cut marks. When Steel needs structured OCR/table extraction or persisted drawing evidence, use PaddleOCR MCP OCR (`PaddleOCR-VL-1.6` / `paddleocr_vl`) as the required text/table parser. Persist the assistant's OCR confirmation Markdown into Steel structured state and reuse persisted OCR Markdown on follow-up turns unless re-OCR is explicitly requested or state is missing/incomplete.
...
4. The configured PaddleOCR MCP implementation, currently `PaddleOCR-VL-1.6` / `paddleocr_vl`, is the primary OCR source for Steel table/drawing extraction.
