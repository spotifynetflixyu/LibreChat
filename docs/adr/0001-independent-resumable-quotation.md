...
Preparation uses saved-data presence rather than a fixed question sequence. Every AI turn receives whether OCR, customer Markdown, and a final system order exist. Explicit B choice is saved from AI customer Markdown; saved customer data is reusable across turns and order revisions. A later signal-only response is bound to the turn's order/customer versions. Existing system orders suppress repeated proactive quote questions without automatically starting new quotations.
