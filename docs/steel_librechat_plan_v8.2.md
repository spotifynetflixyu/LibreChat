...
- 開發階段可在 Steel workspace 內沿用 LibreChat model selector 能力，選擇低成本 OpenAI model。
...
| 3 | Independent Steel Workspace、shared desktop/mobile UX framework、shared API mock data、Quote Resolution Engine、calculators、Workbook JSON、Workbook Preview | 使用者可送訊息並看到七分頁 workbook preview；real OpenAI smoke 驗證 vertical slice |
...
5. 取得 OpenAI conversation id。
6. 組 Prompt Bundle。
7. 呼叫 OpenAI Responses API。
8. 處理 function calling loop。
9. 處理 structured outputs。
10. 寫入 `steel_openai_runs`。
11. 更新 `steel_conversation_meta.openaiMeta`。
12. 回寫 workbook patch / memory candidate / merge table patch。

Orchestrator acceptance：

- OpenAI client 可 injectable，測試不打真實 API。
- Phase 3 可先用 `packages/data-provider/src/steel/mock/` 的 shared API mock data 開發 Chat Workspace / Workbook Preview。
- Mock data 透過 explicit mock path 匯入，不進 `packages/data-provider/src/steel/index.ts` 穩定 export。
- Public workbook DTOs 來自 `packages/data-provider/src/steel/workbooks.ts`；backend Zod validation in `packages/api/src/steel/workbook/schema.ts` 是 accepted patch 的權威。
- Cell marker 只是 UX 顯示；Phase 3 每次 submit 最多一個 `selectedWorkbookRefs` item，後端以 structured ref 驗證欄位目標，不讓 AI 只靠 marker text 猜 patch path。
- 使用者也可以在無 selected ref 或只有一個 selected ref 的情況下，用文字描述多個明確 workbook 修改；AI 產生的每個 patch op 都必須通過 workbook service 驗證。
- Phase 3 不要求每個 AI workbook patch 先顯示 preview/diff 等待使用者確認；server 接受 patch 後，回傳 changed paths 給 UI 標示最新更新欄位。
- 至少一個 manual live smoke test 使用真實 OpenAI API 建立 customer-visible workbook，作為進 Phase 4 前驗證。
...
此 pipeline 只用於報價對話中的圖面/圖片/PDF evidence，不屬於 Admin data import。Admin ERP 匯入只接受 XLSX；鋼鐵手冊 DOCX 是 schema/data-model reference，不是 Admin web upload。
...
11. never write formal source tables without Admin ERP XLSX import, reviewed table UI edit, or a future approved handbook data SQL import
