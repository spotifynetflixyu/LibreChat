...
文件狀態：可執行開發規格；本版改以 openai-oauth /v1/responses 為主要 AI API driver，官方 OpenAI API 只在逐項 smoke test 通過後才可作 secondary routing / fallback

本文件將 v8.2 規劃升級為可交給工程實作的 v8.3 規格。v8.3 的核心變更是：AI API 主要 driver 改為 openai-oauth /v1/responses、採 stateless full-history tool loop、前端 Steel Workspace 預設選擇 openai_oauth_responses、移除第二套 agent harness 規劃、official OpenAI API 不再視為未測保底、所有 fallback capability 都必須先實作並 live smoke test passed。原有固定 Workbook 七分頁、ERP XLSX 作為正式 Admin 匯入來源、鋼鐵手冊 DOCX 先作真實 schema/data-model 設計參考、Admin 不做 DOCX/PDF parser upload path、Quote Workbook vertical slice、MongoDB + Supabase PostgreSQL、Quote Resolution Engine 與 deterministic calculator 等決策保留。
...
- Steel AI Provider adapter 狀態追蹤；預設 driver 為 `openai_oauth_responses`，呼叫 openai-oauth `/v1/responses`；`openai_api` 只作已測通 capability 的 secondary driver。
- 開發階段 Steel workspace 的 model selector 由後端 allowlist 控制；預設選擇 `openai_oauth_responses`，`openai_api` 只在對應 capability smoke test passed 後顯示或可 fallback。
...
| AI Provider              | Steel AI Provider adapter；預設 `openai_oauth_responses` / `/v1/responses`；`openai_api` 為 secondary driver，且僅在對應 capability smoke test passed 後才可 routing / fallback           |
...
| 3     | Independent Steel Workspace、shared desktop/mobile UX framework、shared API mock data、Quote Resolution Engine、calculators、Workbook JSON、Workbook Preview | 使用者可送訊息並看到七分頁 workbook preview；`openai_oauth_responses` text/tool/workbook vertical slice smoke passed；file capability smoke tests 分項紀錄；`openai_api` fallback 不得未測啟用 |
...
- `openai_oauth_responses`：主要開發 driver。第一版以 `openai-oauth-provider` direct provider 為唯一 coded provider path，呼叫 `/v1/responses`。Vercel AI SDK 6 dependency 是 Apache-2.0，已確認可用於 production；實作時必須透過 package-manager overrides/resolutions 統一 `ai`、`@ai-sdk/openai`、`@ai-sdk/provider`、`@ai-sdk/provider-utils` 版本。`openai-oauth` CLI localhost proxy 只作手動 diagnostic smoke probe，不是 runtime route。此 driver 先實作 text、streaming、tool calling、structured output、image、PDF、DOCX、XLS、XLSX 等 smoke tests。
- `openai_api`：第二 driver。使用官方 OpenAI API Key / Responses API。只在已實作且 live smoke test 通過的 capability 上允許 routing 或 fallback；不得把 official API 當成未測保底。
...
- Capability smoke test runner、routing gate、audit 與 typed error handling。
...
```text
openai_oauth_responses：本案主要 AI API driver；先測通再開 capability。
openai_api：第二 driver；也必須先逐項實作與 live smoke test，通過後才可 fallback。
```
...
本案只承認 capability smoke test 結果，不用 provider 名稱或模型名稱推斷能力。
...
```bash
npx openai-oauth@latest --host 127.0.0.1 --port 10531 --models gpt-5.4,gpt-5.3-codex
```
...
| 一般聊天                        | 主要 driver；`/v1/responses` text + streaming smoke test passed 後啟用                                     | 第二 driver；text + streaming smoke test passed 後才可 fallback 或手動切換                              |
| Structured output               | 必須測 workbook patch JSON / Zod validation；未通過不得寫 workbook                                         | 必須測 structured output；未通過不得作 fallback                                                         |
...
| Image input                     | 必須實測 PNG/JPG 圖面；passed 才可處理圖片附件                                                             | 必須實測；passed 才可作圖片 fallback                                                                    |
| PDF input                       | 必須實測文字 PDF、掃描 PDF、圖面 PDF；passed 才可處理 PDF                                                  | 必須實測；passed 才可作 PDF fallback                                                                    |
| DOCX input                      | 必須實測 Word 訂單與表格；passed 才可處理 DOCX                                                             | 必須實測；passed 才可作 DOCX fallback                                                                   |
| XLS input                       | 必須實測舊版 Excel `.xls`；passed 才可處理 XLS                                                             | 必須實測；passed 才可作 XLS fallback                                                                    |
| XLSX input                      | 必須實測 workbook / sheet / header / row evidence；passed 才可處理 XLSX                                    | 必須實測；passed 才可作 XLSX fallback                                                                   |
...
v8.3 不再用單純 boolean 或 `unverified`。每個 driver、model、capability 都要保存狀態：
...
- `implemented_untested` 不可視為可用。
- `failed` 不可自動 fallback 成另一 driver，除非另一 driver 的同一 capability 是 `passed`。
- `openai_api` 沒有該 capability 的 `passed` 記錄時，不得被當作保底。
...
- 需要 backend API tool calling：優先 `openai_oauth_responses`。若 tool loop smoke test 失敗，只有 `openai_api.toolCalling=passed` 時才可 fallback。
- 需要圖片、PDF、掃描圖面、OCR、孔洞/折線/開槽判讀：先看 `openai_oauth_responses.imageInput/pdfInput` 是否 `passed`。若未 passed，再看 `openai_api.imageInput/pdfInput` 是否 `passed`。兩者皆未 passed 時，不做 provider fallback，該附件標低信心並列人工複核。
...
- 需要 hosted File Search / Code Interpreter：只有該 driver 的 hosted tool smoke test passed 才可用。否則走 backend retrieval / deterministic parser / 人工複核，不可假設 official API fallback。
- provider auth failed、subscription limit、rate limit、event unsupported、file input unsupported、structured output invalid 時，必須分類錯誤、寫 audit、按 capability gate 決定是否 fallback。
...
- Steel Workspace 預設選擇 `openai_oauth_responses` / `/v1/responses`，但只在 `text + streaming` passed 時設為 enabled。
...
- `openai_api` 不因存在 API key 就自動顯示為可用；必須有至少一項 capability passed 或 admin 明確開啟「待測模式」。
- 未通過 tool calling / structured output smoke test 的模型不得進報價 runtime。
- 未通過 file / vision / XLS/XLSX smoke test 的模型不得處理附件報價。
- UI 要顯示 typed error，例如「PDF capability 尚未通過 smoke test，已列入人工複核」，而不是靜默 fallback。
...
- 對使用者顯示用量時，OAuth driver 顯示「subscription driver 狀態 / 最近錯誤 / 是否 fallback」；API driver 顯示「token usage / estimated cost / rate limit headers / budget status」。
...
說明：`STEEL_OPENAI_OAUTH_ALLOW_PRODUCTION=true` 只適用於 direct provider path，且仍需通過 dependency overrides/resolutions、packaging verification、server-side auth storage、capability smoke gates。Direct provider 不需要 `/v1` base URL 或 transport selector；local proxy 只作 local/dev manual smoke probe，不可作 hosted multi-user service，也不進 runtime route。`STEEL_OPENAI_OAUTH_AUTO_FALLBACK=false` 表示 OAuth primary 失敗或 capability 不支援時，不自動 reroute 到 `openai_api`；即使開啟，也必須要求對應 `openai_api` capability smoke result 是 `passed`。`STEEL_FALLBACK_ON_*` 預設 false，直到 secondary driver 的對應 capability 實作且 smoke test passed 才允許開啟。
...
3. 取得 selected model 與 `STEEL_AI_DRIVER_DEFAULT`。
...
6. 讀取 driver capability matrix；primary driver 未 `passed` 時，不直接 fallback，先檢查 secondary driver 是否同 capability `passed`。
...
- 必測 smoke cases：純文字聊天、streaming、tool call、tool result round-trip、structured workbook patch、image、PDF、DOCX、XLS、XLSX。
...
- 只在 `openai_api` 對應 capability 已實作且 live smoke test passed 時，才可作 secondary route 或 fallback。
...
此 pipeline 只用於報價對話中的圖面/圖片/PDF/DOCX/XLS/XLSX evidence，不屬於 Admin data import。Admin ERP 匯入只接受 XLSX；鋼鐵手冊 DOCX 是 schema/data-model reference，不是 Admin web upload。
...
official OpenAI API fallback 不可未測假定；所有 fallback capability 必須先通過 smoke test。
```
...
5. `CapabilityGate` 優先檢查 `openai_oauth_responses`；若未 passed，再檢查 `openai_api`；若 secondary 也未 passed，不 fallback，回 typed error 並建立人工複核項。
...
- 孔洞、開槽、折線、切角、尺寸不清。
- `openai_oauth_responses` 未通過 file / vision / XLS/XLSX smoke test。
- `openai_api` 對應 fallback capability 未實作、未測、failed 或 disabled。
...
- 在未通過 capability smoke test 前，將 official OpenAI API 當作 PDF、圖片、DOCX、XLS、XLSX 的 fallback。
...
- [ ] LibreChat Steel Workspace model selector 預設選擇 `openai_oauth_responses`，但僅顯示已通過 smoke test 的模型與能力，並沿用 LibreChat model/default setting framework 解析預設 model 與 runtime settings。
...
- [ ] `openai_api` driver 已實作但不當作未測保底；只有對應 capability smoke test passed 才可 fallback。
- [ ] official API PDF / image / DOCX / XLS / XLSX / hosted tool smoke tests 分別紀錄，不以存在 `OPENAI_API_KEY` 代表可用。
- [ ] fallback routing 使用 `STEEL_FALLBACK_REQUIRE_CAPABILITY_PASSED=true`。
- [ ] fallback capability 未 passed 時回 typed error，建立低信心或人工複核項，不靜默重試。
- [ ] tool calling loop 有 max call guard。
...
9. 建立 driver capability smoke test：text、streaming、tool calling、structured output、image、PDF、DOCX、XLS、XLSX、File Search、Code Interpreter。
...
15. 若 primary driver 對 file / vision / XLS / XLSX / hosted tools 失敗，只在 secondary driver 同 capability passed 時 fallback。
16. 若 secondary 未測通，不 fallback，建立 typed error、低信心與人工複核項。
...
判斷：採用。v8.3 將 `openai_oauth_responses` 設為 primary driver，但只承認已通過 smoke test 的能力。

```text
「先測試可呼叫、傳圖片、傳 PDF、傳 DOCX、傳 XLS、XLSX」
```

判斷：採用。這些都列入 capability smoke test。未測或 failed 不可進 runtime。
