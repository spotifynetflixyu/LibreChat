...
| 3     | Independent Steel Workspace、shared desktop/mobile UX framework、shared API mock data、Quote Resolution Engine、calculators、Workbook JSON、Workbook Preview | 使用者可送訊息並看到七分頁 workbook preview；`openai_oauth_responses` + `gpt-5.5` text/tool/workbook vertical slice evidence passed；`openai_api` 不得作未決策保底 |
...
MVP runtime 流程由 AI 先查任務範圍內的 instruction packets，再判斷訂單鋼材類別、可能鋼材種類、表面處理、尺寸與查表候選 query，接著呼叫上述 reviewed lookup tools。Backend tools 回傳有來源、有界的 instruction/customer/price/default/formula 候選資料；AI 再從候選中推導最可信的價格與公式路徑，產生 workbook 內容、說明、選項與待確認事項。

Agent Instruction 是 Admin-managed default instruction，每輪 Steel quote turn 都內建注入。它負責全域流程、工具使用、安全與來源驗證規則，可由 Admin 後台更新與版本化，不應寫死在 provider adapter。規劃儲存在 `steel.agent_instructions`。`lookup_instructions` 則查任務範圍內的細部 instruction packets，規劃儲存在 `steel.instruction_packets`。

Agent Instruction 可包含 Steel 訂單推導的全域規則：OCR / 圖片與 PDF 方向判斷、繁中保留、圖面與表格優先序、工具路由、訂單拆行、缺欄位確認、workbook provisional/confirmed 寫入規則、來源驗證與低信心回覆。細部鋼材或加工規則仍可拆成 task-scoped instruction packets 由 `lookup_instructions` 查回。
...
Workbook 目前透過 provider-facing `patch_workbook` output tool 更新。當 workbook context 存在時，AI 可呼叫 `patch_workbook` 產生 typed operations；backend 再用 workbook schema/service 驗證與套用。`patch_workbook` 不是 reviewed lookup tool，不應和 `search_price_candidates` / `lookup_defaults` 這類查資料工具混在同一個 MVP lookup list。

`docs/reference/instruction.txt` 是目前 instruction seed source。Runtime 不應每次把整份 instruction 檔注入 prompt；應透過 `lookup_instructions` 取回與任務相關的 reviewed packets，例如價格先於重量、口語品名轉換、C 型鋼規則、長條料配料、切工、孔洞、開槽、折工、圖片/PDF 判讀與 workbook 輸出要求。

不列為 MVP runtime tools：

- `normalize_quote_item`、`generate_price_search_terms`、`rank_price_candidates`：AI reasoning / backend internal validation，不是 tool。
- `lookup_customer`：由 `search_customers` 回傳 exact 或 ambiguous customer candidates。
...
Ranking is backend internal decision policy plus AI explanation. It is not an
exposed `rank_price_candidates` MVP tool.
...
### 18.3 calculate_cut_count / calculate_cutting_fee
...
- 先算 `operationCutCount`，再算 `billableCutCount`；前者給實際加工/系統訂單，後者給報價收費。
- 一支素材切出 `n` 支成品且剛好用完、不切尾時，分離刀數 = `n - 1`。
- 一支素材切出 `n` 支成品且有餘料時，分離刀數 = `n`，因最後一支成品仍需與餘料分離。
- 「有餘料則省略切尾」只省略額外尾修/整尾刀，不省略成品與餘料之間的分離刀。
- 切頭每支素材加 1 刀；切尾每支素材加 1 刀，但有餘料且規則允許省略切尾時不加尾修刀。
- C 型鋼可依選定規則將 `billableCutCount` 設為 0，但仍可記錄 `operationCutCount` 供系統訂單或備註使用。
...
- `4-Ø22` = 每片 4 孔。
- 1 個圓孔、橢圓孔、長孔、長方孔或其他非圓孔算 1 孔，除非規則另定。
- 總孔數 = 每片 / 每支孔數 x 數量。
- 中心線、尺寸線、虛線、R角、折線、切角、焊接符號不可誤判為孔。
- AI 先抽出 hole groups：孔型、圓孔孔徑、非圓孔長寬或文字尺寸、每片/每支孔數、數量倍率、來源與信心。
- 後台未來新增橢圓孔、長孔、長方孔或 custom 非圓孔價格時，runtime 查價與 calculator 必須可直接使用 reviewed DB 資料。
- 孔數、孔型、孔徑或數量倍率不清時不可當 0，需列人工複核或詢問使用者。
- C 型鋼孔費預設 true zero 只來自選定計算規則；孔數仍可留在判讀備註。
...
- 多條不相連路徑分別加總。
- 路徑不明不可當 0，需低信心。
- AI 先抽出 slot paths：直線、L、U/ㄇ、多段或未知；各段長度、每片路徑數、數量倍率、來源與信心。
- 邊緣開槽仍需計算；但普通外框、一般板邊或外形輪廓不可自動當成開槽。
...
所有圖片、掃描 PDF、拍照圖面、訂單截圖、手寫單、材料表圖片，在讀文字前必須先由 AI 判斷方向；若需要裁切、旋轉、放大、影像處理，優先使用 backend support matrix 允許的 provider 能力或 backend deterministic image preprocessing，不可假設任何 driver 都可用 hosted tools。
