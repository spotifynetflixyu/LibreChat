...
- `openai_oauth_responses`：主要開發 driver。使用 `openai-oauth` CLI localhost proxy 的 OpenAI-compatible base URL，呼叫 `/v1/responses`。定位是 subscription-backed local / dev-first runtime，先以此 driver 實作 text、streaming、tool calling、structured output、image、PDF、DOCX、XLS、XLSX 等 smoke tests。
...
說明：`STEEL_FALLBACK_ON_*` 預設 false，直到 secondary driver 的對應 capability 實作且 smoke test passed 才允許開啟。
...
- [ ] LibreChat Steel Workspace model selector 預設選擇 `openai_oauth_responses`，但僅顯示已通過 smoke test 的模型與能力。
...
GET    /api/steel/admin/tables/:targetTable/rows
POST   /api/steel/admin/tables/:targetTable/preview-update
POST   /api/steel/admin/tables/:targetTable/commit
...
4. base URL 預設 http://127.0.0.1:10531/v1，可由 env 覆蓋。
...
7. 完成 server-side OAuth login / token store；token 檔視為密碼等級保存。
8. 建立 driver capability smoke test：text、streaming、tool calling、structured output、image、PDF、DOCX、XLS、XLSX、File Search、Code Interpreter。
9. 建立 /api/steel/conversations/:conversationMetaId/messages route 到 SteelAgentOrchestrator。
10. 使用者上傳檔案後，backend 建立 source file metadata 與 provider payload；AI 做判讀與 evidence，不由 Node 寫主 OCR/圖面 parser。
11. Steel tools 仍由 backend 執行；tool decision 由 AI，tool execution 由 Steel backend。
12. 將 provider stream/tool events 轉成 SteelAIEvent。
13. 測試純聊天、backend API tool call、structured output、workbook patch、圖片圖面、PDF、DOCX、XLS、XLSX evidence。
14. 若 primary driver 對 file / vision / XLS / XLSX / hosted tools 失敗，只在 secondary driver 同 capability passed 時 fallback。
15. 若 secondary 未測通，不 fallback，建立 typed error、低信心與人工複核項。
16. run localhost Steel Workspace page，確認預設聊天由 openai_oauth_responses driver 執行，fallback 事件可追蹤。
```
